...
- 2026-08-31 physical-device retry: CoreDevice reports the new iPhone (iPhone18,3, iOS 26.6.1) as available and paired, and `xcodebuild -showdestinations` exposes it as a physical iOS destination. A valid Apple Development signing identity is installed. The signed build correctly stopped before compilation because Developer Mode is disabled on the phone; enable it under Settings → Privacy & Security, restart, unlock, and confirm before retrying installation.

A physical iPhone was connected for destination/signing readiness only. The app has not yet been installed or run on it, and no BLE peripheral or FPGA was used in this review.
...
- Result: iOS software commit `3aaf1fb` and test/status commit `973d63e` are accepted on `main`; readiness evidence is recorded at `3297905`. Transport software handoff `1b72824`, all gameplay/video software, and Board A structural integration are accepted on `main`. The new iPhone is paired and visible to Xcode, but Developer Mode is disabled, so the signed app cannot yet be installed. No programmed-board bitstream exists and BLE GATT values remain unknown. Therefore no advertised name, service/characteristic UUID, characteristic property, notify UUID, maximum write length, one-byte UART result, or 50 Hz counter result can yet be measured truthfully.
...
- Remaining prerequisites: enable Developer Mode on the connected iPhone, restart/unlock/confirm it, and install the signed app; make a programmed Boolean Board and transport bring-up bitstream available; then perform GATT discovery, the `41 0A` one-byte check, and both two-minute 50 Hz C1 runs.
...
- The new paired iPhone is now a selectable Xcode destination, but Developer Mode is disabled. Enable it and complete the required restart/confirmation before signing, installation, or device tests can proceed.

## Next action

Enable Developer Mode on the connected iPhone, retry signed build/install and Core Motion verification, then discover and record the verified Boolean Board GATT interface when a programmed board is available. Coordinate with the transport owner for the one-byte UART check and two-board C1 runs. Do not mark C1 passed from simulator or device-only evidence.
